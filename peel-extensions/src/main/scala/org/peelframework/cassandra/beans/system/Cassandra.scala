package org.peelframework.cassandra.beans.system
...
import org.peelframework.core.config.Model.Env

import scala.collection.JavaConverters._

/** Wrapper class for Cassandra.
  *
  * Implements Cassandra as a Peel [[org.peelframework.core.beans.system.System System]].
  *
  * @param version Version of the system (e.g. "7.1")
  * @param lifespan [[Lifespan Lifespan]] of the system
...
class Cassandra(version: String, lifespan: Lifespan, dependencies: Set[System] = Set(), mc: Mustache.Compiler) extends System("cassandra", version, lifespan, dependencies, mc) {
  ...
  override def configuration() = SystemConfig(config, {
    ...
    List(
      SystemConfig.Entry[Env]("system.cassandra.config.env", s"$conf/cassandra-env.sh", templatePath("conf/cassandra-env.sh"), mc),
    ...
    while (!isUp) {
      try {
        ...
        while (curr - init < totl) {
          ...
          // get new values
          val cmd = s"""${config.getString("system.cassandra.path.home")}/bin/nodetool -h $pollingNode -p $port status | grep 'UN' | wc -l"""
          curr = Integer.parseInt((shell !! cmd).trim())
